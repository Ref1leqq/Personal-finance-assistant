    ...
    def add_goal_window(self):
        ...
        def save_goal():
            ...
            try:
                ...
                return  # Прерываем сохранение, если данные некорректны
...
def check_goal_title(title, testing=False):
    ...
    """
    if not title.strip():  # Убираем лишние пробелы и проверяем пустоту
        if testing:
...
def check_goal_amount(target_amount, testing=False):
    ...
    """
    if not target_amount.strip():  # Проверка на пустую строку
        if testing:
    ...
    if not target_amount.isdigit() or int(target_amount) <= 0:  # Проверка на положительное число
        if testing:
...
def check_goal_date(target_date, testing=False):
    ...
    """
    if not target_date.strip():  # Проверка на пустую строку
        if testing:
